...
# CORPUS
====================================================================================================
# CORPUS ASSEMBLY — what the repository can and cannot establish

**Bottom line: of the six items, 2 and 5 are fully recoverable; 3 is partly recoverable (only as "what the artifact shows", not "what the procedure was"); 6 is recoverable and the answer is *no*; 1 and 4 are unrecoverable from this repository.** No script, notebook, or deleted file in this repository or in any of its 844 commits reads ChEMBL, DrugBank, MetXBioDB, or the GLORYx reference set, and none writes `{train,val,test}.sdf` or the base `*_triples.txt`.

---

## 0. The load-bearing physical facts about the shipped corpus

The files are gitignored symlinks into the main checkout. Their mtimes are decisive:

```
train.sdf            1,191,859,108 B   2024-09-10 06:11:43
train_triples.txt        6,042,236 B   2024-09-10 06:13:05
test.sdf               138,891,883 B   2024-09-10 06:14:40
test_triples.txt           636,521 B   2024-09-10 06:15:27
val.sdf                134,996,965 B   2024-09-10 06:16:22
val_triples.txt            585,199 B   2024-09-10 06:16:37
train/val/test_triples_clean.txt       2026-03-28 08:58:42  (all three, same second)
```

All six primary files were written by one run inside five minutes on **2024-09-10**, eight months before this repository's first commit (`554bed3`, 2025-05-06 20:25:37). The producer is not in the repository and never was.

SDF record structure (measured over all three files):

| | records | `ID` | `State` values |
|---|---|---|---|
| train | 378,241 | **empty in all 378,241** | `0`: 339,132 · `1`: 29,030 · `Substrate`: 10,079 |
| val | 41,591 | **empty in all 41,591** | `0`: 37,219 · `1`: 3,261 · `Substrate`: 1,111 |
| test | 45,134 | **empty in all 45,134** | `0`: 40,186 · `1`: 3,700 · `Substrate`: 1,248 |

Per-record fields are exactly `ID`, `InChI`, `State`, `SMILES`, `Index`. **`ID` — the only field that could have carried a source accession — is empty in every one of the 464,966 records.** This single fact is why item 4 is unrecoverable. `State` is used by analysis code (`scripts/transfer_stratified.py:59`, `scripts/ceiling_gap_by_similarity.py:54`, `scripts/typed_edit/dialect_census.py:274`) but is documented nowhere; its semantics are `Substrate` / `1` = annotated metabolite / `0` = rule-applicable unannotated product. I verified the mapping is exact: over all 3,700 test positive rows, every substrate id has `State=Substrate` and every product id has `State=1`.

---

## 1. Per-source extraction (query, filter, fields kept) — **UNRECOVERABLE**

Evidence that it is absent, not merely hard to find:

- Pickaxe over all refs for every spelling of the source names (`git log --all -S`) returns **only prose commits**. `ChEMBL`/`MetXBioDB` appear first in `8e7bf58` (2026-06-28, `docs/benchmark/protocol.md`, a datasheet) and then only in `paper2/` commits. `chembl`, `CHEMBL`, `drugbank`, `metxbio` (lowercase/accession forms) have **zero hits in the entire history**. `chembl_webresource`, `sqlite3`, `drugbank.xml` have zero hits.
- The two commits that introduced the four-source claim and the versions — `69fc2fd` "the corpus is named, and GLORYx was never an external set" and `ea240c6` "the corpus versions…" — touch only `body.tex`, `manuscript_draft.md`, `markers.tex`, `refs.bib`, `si.tex`, `zenodo_manifest.json`, `zenodo_deposit.py`. **No script, no artifact, no data file.** "ChEMBL 34, DrugBank 5.1.10, MetXBioDB 1.0, GLORYx 2020" is an author assertion typed into the manuscript; it is not derived from anything in the repository, and `scripts/check_paper2_numbers.py:48` whitelists `"34"` as "a release name" rather than checking it against a source.
- Every deleted notebook was inspected. `notebooks/Unit_Tests.ipynb`, `parameter_tuning.ipynb`, `generator_tuning.ipynb` **consume** `train.sdf`/`*_triples.txt` via `MolFrame.from_file(..., standartize=False)`. None produces them. `uspto_process.ipynb`, `reduce_reactions.ipynb`, `scripts/legacy/uspto_processer_{1,2}.py` are the USPTO reaction-template pipeline, unrelated to the metabolite corpus.
- `SDWriter`, `MolToMolBlock`, `write_sdf`, `to_sdf` over all refs: the **only** hit is `086fad0` → `scripts/make_metatox_input.py:88`, which writes a 291-substrate MetaTox submission file, not the corpus.
- The repository states this itself: **`docs/REPRODUCIBILITY.md:12`** — "Primary raw-data extraction before SDF creation is intentionally out of scope." **`docs/DATASETS.md:15-16`** — "The repository treats these as external research data."

**What would establish it:** the 2024-09-10 producer script/notebook from the author's machine, or an assembly log. Nothing short of that. Note also that the repository's older documentation contradicts the current manuscript on provenance: `docs/benchmark/manuscript.md:991-995` says the data is **"DrugBank-derived"** (single source), and `docs/STATUS.md:330` lists "Ingest external metabolism corpora (DrugBank/HMDB metabolism, MetaXBioDB, GLORYx training reactions)" as **future work**. Both predate the four-source claim.

---

## 2. Standardisation at assembly time — **RECOVERABLE, and the answer is: the stored SMILES are standardiser output, not raw source drawings**

`grail_metabolism/utils/preparation.py:127-157` — `standardize_mol` = `rdMolStandardize.Cleanup` (135) → `FragmentParent` (136) → `Uncharger.uncharge` (139) → `TautomerEnumerator.Canonicalize` (155) → `_normalize_return_type` (120-121) which emits `Chem.MolToSmiles(mol, isomericSmiles=False)`.

Four independent measurements on the shipped `test.sdf` say the stored strings passed through exactly that pipeline:

1. **Stereo:** 0 of 300 sampled substrates and 0 of 300 sampled annotated products contain `@`, `/`, or `\`. Matches `isomericSmiles=False` at line 121.
2. **Canonicality:** 300/300 substrates and 300/300 products are already byte-identical to `Chem.MolToSmiles(MolFromSmiles(s), isomericSmiles=False)`.
3. **Tautomer signature:** 77/300 substrates (25.7%) are *not* fixed points of today's `standardize_mol`, and in **77/77 cases the molecular formula is unchanged** — every difference is a tautomer shift, specifically iminol → amide, e.g. stored `…C(O)=Nc3c(C)cccc3C…` → today `…C(=O)Nc3…`; stored `N=C(O)CN=C(O)C1CCCN1C(=O)CN` → today `NCC(=O)N1CCCC1C(=O)NCC(N)=O`. Raw ChEMBL/DrugBank drawings are not written as imidic acids. This is `TautomerEnumerator.Canonicalize` output from an **older RDKit** than the 2024.09.6 installed here. The repository already documents this artefact independently: **`scripts/make_metatox_input.py:9-12`** — "GRAIL's substrate SMILES come out of `standardize_mol` tautomer-canonicalized, which for ~27% of this set stores an UNNATURAL iminol/imidic-acid tautomer"; commit `086fad0` re-tautomerised 79 of them before external submission.
4. **The `InChI` field is derived, not source:** `Chem.MolToInchi(MolFromSmiles(stored_SMILES))` equals the stored `InChI` in **500/500** sampled records. The InChI is a function of the standardised SMILES.

Also assembly-time, not inference-time: **the PU negatives are baked into the SDF.** `State=0` records are pre-computed rule-applicable products; at load `MolFrame._init_from_dataframe` (`preparation.py:605-610`) routes `real=1` rows into `map` and `real=0` rows into `gen_map`, and `negatives()` (`:876-879`) just differences them. No rules are applied at load.

**Which bank produced those negatives is not recorded.** I probed it: 15 random test substrates, shipped candidate pool 315 structures, each bank applied via `apply_rules_to_molecule` with both sides re-standardised under today's enumerator so tautomer drift cancels:

| bank | rules | covers shipped pool | generates | of which **not** in shipped |
|---|---|---|---|---|
| `grail_metabolism/data/smirks.txt` | 474 | 293/315 | 308 | **15** |
| `data/merged_smirks.txt` | 656 | 294/315 | 425 | 131 |
| `resources/notebooks_rules.txt` | 1051 | 300/315 | 342 | 42 |
| `resources/extended_smirks.txt` (ships) | 7581 | 303/315 | 1442 | 1139 |
| `data/xtracted.txt` | 4534 | 58/315 | 656 | 598 |
| `resources/mined_only.txt` | 5866 | 78/315 | 1109 | 1031 |

The banks are strictly nested (474 ⊂ 656 ⊂ 1051 ⊂ 7581, verified by set containment). The shipped negatives are consistent with the **474-rule curated bank**, and flatly inconsistent with the 7,581-rule publication bank, which over-generates by 4.7×. This is a 15-substrate probe, so treat it as strong evidence rather than proof; a full-corpus reproduction per bank would settle it.

**One caveat for the Methods text:** `DatasetConfig.standardize` defaults to `True` (`config.py:41`) and `workflows/data.py:102` passes it to `MolFrame.from_file`, which re-runs `_standardize_smiles_cached` on already-standardised strings (`preparation.py:702-708`). Under the current RDKit that silently *re-tautomerises about a quarter of the corpus at load time*. The historical notebooks used `standartize=False`. Whatever is written about standardisation should say which of the two the reported runs used.

---

## 3. Cross-source de-duplication and conflict rule — **PARTLY RECOVERABLE (artifact only; no procedure exists)**

There is no de-duplication or conflict-resolution code anywhere, and with `ID` empty there is no source field a conflict rule could have arbitrated on. What the artifact shows:

- **The SDF is not structure-deduplicated.** test: 45,134 records, 39,602 distinct SMILES. val: 41,591 records, 36,621 distinct. Among `State=Substrate` records: test 1,248 records → 1,200 distinct structures; val 1,111 → 1,063. `Index` is contiguous 1..N in both files.
- **Where a substrate structure repeats under two `Index` values, its annotation set is identical:** 30 duplicated substrate structures in test, **30/30 with identical metabolite sets, 0 conflicts**. So the duplication is redundant blocks, not disagreeing sources.
- **No row is labelled both ways.** Over the clean triples at structure level, pairs listed as both positive and negative for the same substrate: **0** in test, **0** in val.
- **The effective de-duplication is at load, and the effective conflict rule is set union**: `preparation.py:605-610` accumulates into `defaultdict(set)`, so duplicate rows collapse and any two annotation sets for the same standardised substrate merge. There is no precedence.
- This is why the split table's substrate counts differ from the evaluated population: test is **1,198** substrates counted over ids (`results/leakage_fix_report.json`, `paper2/si_table_splits.tex:9`) and **1,170** counted over structures — I measured 1,170 distinct substrate structures carrying 2,597 distinct positive structure-pairs directly from `test.sdf` + `test_triples_clean.txt`, matching `evaluated_test_set` in `paper2/split_manifest.json` exactly.

---

## 4. Per-source counts — **UNRECOVERABLE as recorded; partially *re-derivable* for two of the four sources**

Nothing under `results/` (308 files), `artifacts/`, `strata/`, or `configs/` mentions ChEMBL, DrugBank, or MetXBioDB. `results/` has no source-attribution artifact of any kind. Combined with the empty `ID` field, per-source counts cannot be read off anything.

Two of the four sources are physically present in the working tree, so an *overlap* measurement is possible. I ran it (InChIKey-skeleton keys, over the clean splits):

| | corpus pairs | corpus substrates |
|---|---|---|
| **corpus total** (train+val+test, clean, distinct skeleton pairs) | 22,134 | 10,972 |
| ∩ **MetXBioDB 1.0** (`artifacts/tier2/biotransformer/database/MetXBioDB-1-0.json`, 2,110 biotransformations → 2,035 distinct pairs, 1,205 substrates) | **1,248** (5.6% of corpus; 61.3% of MetXBioDB) | 901 |
| ∩ **GLORYx test set** (`docs/benchmark/data/gloryx_test.json`, 37 parents → 136 pairs) | **63** of 136 | **23** of 37 |

per split — MetXBioDB: train 917 pairs / 675 substrates, val 133 / 94, test 199 / 133. GLORYx: train 50 / 18, val 11 / 4, test 2 / 1.

Two things this establishes and one it does not. It establishes that **the assembly did not ingest MetXBioDB wholesale** — 787 of its 2,035 pairs are absent from the corpus, so a filter of unknown shape dropped ~39%. It corroborates the manuscript's GLORYx claim (independently, `results/external_overlap_audit.json` records 24 of 37 GLORYx parents in train-or-val under the tautomer key; I get 23 across all three splits under the skeleton key — the small gap is the key, not a disagreement). It does **not** attribute any pair to a source: a pair in MetXBioDB may equally have entered via DrugBank. ChEMBL and DrugBank are not present in the tree and cannot be measured at all.

Note for the licence question: MetXBioDB 1.0 exists here only inside the **untracked, gitignored** BioTransformer checkout (`.gitignore:26` excludes `artifacts/`); the tracked `grail_metabolism/resources/external/` copies are the BT *reaction template* files, not MetXBioDB.

---

## 5. Splitting algorithm and seed — **RECOVERABLE, and the answer is: `fix_splits.py` REPAIRS; the partition itself has no producer**

### `fix_splits.py` only repairs

`scripts/fix_splits.py:20-30` takes `{train,val,test}.sdf` + `*_triples.txt` as **inputs** and writes only `*_triples_clean.txt` (`:298-300`). It never assigns a substrate to a split; it deletes rows from an already-existing partition. `:300` is `shutil.copyfile` — test is passed through untouched, and indeed `test_triples.txt` and `test_triples_clean.txt` are **byte-identical** (`sha256 5989734ab81d45ad…` for both).

### I reproduced the shipped clean files, and they came from the DEFAULT mode, not `--molecule-disjoint`

Read-only re-run of `fix_splits`' own functions against the shipped SDFs:

| mode | file | reproduced | shipped | match |
|---|---|---|---|---|
| substrate (default) | **val** | 37,959 rows, `sha 669a0346e9cc542e…` | 37,959, `sha 669a0346e9cc542e…` | **byte-identical** |
| substrate (default) | train | 345,028 rows | 345,031 | 3 rows short, order identical, strict subset |
| `--molecule-disjoint` | val | 35,402 | 37,959 | no |
| `--molecule-disjoint` | train | 310,062 | 345,031 | no |

The val digest also matches `paper2/split_manifest.json` → `files.val.sha256`. The three missing train rows are exactly `(104954,104969,0)`, `(324954,324990,0)`, `(324991,325026,0)` — all label 0, and they correspond to the "invalid SDF records: 3, missing ids: 3" the loader reports: three `train.sdf` records that today's RDKit no longer parses. The reproduction is a strict subset in identical order, with nothing extra.

**So the shipped splits are substrate-disjoint only.** Three places in the repository say otherwise and are stale: `CLAUDE.md` ("`python scripts/fix_splits.py --molecule-disjoint   # regenerate leakage-free clean split triples`"), `docs/benchmark/protocol.md:32-36` ("Splits are **molecule-disjoint**… Built and audited by `scripts/fix_splits.py`"), and `scripts/audit_leakage.py:3` ("`fix_splits.py --molecule-disjoint` BUILDS the clean splits"). `results/leakage_fix_report.json` itself records `"molecule_disjoint": false` with molecule overlaps 2,251 / 2,408 / 700, and `paper2/si_table_splits.tex` and `body.tex` correctly say substrate-disjoint. The manuscript is right; the three notes are wrong.

### The seed: there is none

There is **no split seed anywhere.** `DatasetConfig` (`config.py:28-52`) has no split parameter; `sampling_seed: int = 42` (`config.py:50`) is consumed only by the `max_*_substrates` subsampling (`workflows/data.py:251-267`). `ExperimentConfig.seed = 42` (`config.py:262`) seeds model training. `fix_splits.py` is fully deterministic and imports no RNG.

`MolFrame.train_val_test_split(frac, seed=42, enforce_disjoint=False)` exists at `preparation.py:743-…`, but (a) it returns in-memory `MolFrame`s and writes no file, and (b) its ancestor at the repository's first commit (`748a339:GRAIL/utils/preparation.py:459-476`) used **unseeded** `np.random.shuffle`. It cannot have produced the 2024-09-10 files, which predate the repository. The observed proportions (10,079 / 1,111 / 1,248 substrate records ≈ 81/9/10 %) are in the neighbourhood of `frac=0.1`, but `frac=0.1` on 12,438 substrates predicts 1,243/1,243 for val/test and the observed val is 1,111 — **I am not able to say what algorithm produced the partition, and this ratio should not be presented as if I could.**

**What would establish it:** the 2024-09-10 producer. Absent that, the honest Methods sentence is that the partition is a fixed historical artifact carried forward by digest, repaired for cross-split contamination by `fix_splits.py` in its substrate-level mode.

---

## 6. The split manifest, and whether it lets a third party confirm a rebuild — **RECOVERABLE, and the answer is NO**

`paper2/split_manifest.json`, written by `scripts/typed_edit/freeze_split.py`. It pins, by SHA-256: the three `*_triples_clean.txt` files; the eight stratum files; the rule bank (`extended_smirks.txt`, 7,581 lines); the BioTransformer jar and its three template files; the evaluated test set; and it embeds the cross-split audit read from `results/leakage_fix_report.json`.

It does **not** support a rebuild check, for three concrete reasons:

1. **The SDFs are not pinned at all.** `build()` (`freeze_split.py:137-150`) digests only the triples files. `train.sdf`, `val.sdf`, `test.sdf` — the *only* place the structures live — appear nowhere in the manifest. Every structure in the corpus is outside the fingerprint.
2. **The "substrate set" digests are digests of integers, not structures.** `substrates_of()` (`:64-72`) appends `parts[0]` — the substrate **Index** from the triples file. So `splits.train.substrates.sha256` over `n: 9339` is a hash of ID strings. Swap the SDF for a different set of molecules under the same indices and the digest is unchanged. (Relatedly, `"triples": len(subs)` at `:148` counts rows, not substrates — hence `train: substrates.n 9339` beside `triples 345031`.)
3. **The one structural digest cannot be recomputed without the withheld file.** `evaluated_test_set` (`:151-160`) uses `load_test_map(None, 42)`, which does read structures from `test.sdf` (`run_benchmark.py:76-97`) — so `substrates.n 1170 / sha 85a1cec1…` and `pairs.n 2597 / sha 22c33beb…` are genuine structure digests. But `test.sdf` is not distributed, and the SI states the annotated metabolites are released only as tautomer-canonical InChIKeys. A third party therefore cannot regenerate the input to that digest.

`freeze_split.py --verify` (`:230-241`) will tell an *author* that a leaf moved. It cannot tell a *reader* whether a corpus rebuilt from ChEMBL 34 + DrugBank 5.1.10 + MetXBioDB 1.0 + GLORYx 2020 is the same corpus — which is precisely what `paper2/si.tex:51-53` already concedes: "the split manifest can only prove that a rebuilt corpus differs, not how."
...
|---|---|---|---|
| 1 | per-source extraction: script, query, filters, fields | **unrecoverable** | absent from tree and from all 844 commits; `docs/REPRODUCIBILITY.md:12` declares it out of scope; source names + versions are author assertions added in prose-only commits `69fc2fd`, `ea240c6` |
| 2 | standardisation at assembly | **recoverable** | `preparation.py:127-157`; stored SMILES are stereo-free, RDKit-canonical, and carry the iminol tautomer signature in 25.7% of substrates; `InChI` field = `MolToInchi` of the stored SMILES in 500/500; PU negatives pre-computed, most consistent with the 474-rule `data/smirks.txt` |
| 3 | cross-source dedup + conflict rule | **partly** — no procedure exists | not structure-deduplicated (test 1,248 substrate records → 1,200 structures); 30/30 duplicate substrates agree; 0 positive/negative conflicts; effective rule at load is set union (`preparation.py:605-610`) |
| 4 | per-source counts | **unrecoverable** | `ID` empty in all 464,966 SDF records; nothing in `results/`. Re-derivable overlaps only: MetXBioDB 1,248 pairs / 901 substrates (61.3% of MetXBioDB, 787 of its pairs absent); GLORYx-test 63/136 pairs, 23/37 parents |
| 5 | splitting algorithm + seed | **recoverable** | `fix_splits.py` repairs only; default substrate-level mode reproduces shipped val **byte-for-byte** and train to within 3 RDKit-unparseable rows; `--molecule-disjoint` does not; **no split seed exists**; the partition's producer predates the repository (2024-09-10) and is absent |
| 6 | split manifest sufficiency | **recoverable → no** | `freeze_split.py:64-72, 137-160`; SDFs unpinned, substrate digests are of integer IDs, the one structural digest needs a file that is not released |
