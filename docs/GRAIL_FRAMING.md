...
§2–§4 apples-to-apples and reproducible, not to rank submissions. Every headline stat below ships
with its full provenance; the §2–§4 diagnostic numbers regenerate from committed checkpoints + the
symlinked dataset via `scripts/regen_headline.sh`, and the §Supplement match-protocol numbers —
including the declared primary endpoint — via the `run_match_sensitivity.py` + `rank_flip_ci.py`
harness.
...
- **One-command regen** — `scripts/regen_headline.sh` regenerates the §2–§4 diagnostic reports
  ...
  checkpoints + the symlinked dataset. (The §Supplement match-protocol numbers, including the primary
  endpoint, regenerate separately via `run_match_sensitivity.py` + `rank_flip_ci.py`.) The dominant cost is `factorize_recall.py` (~90 min: deployed
