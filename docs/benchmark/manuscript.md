...
choice; that triad is where we differ in protocol, not in the fact of comparison. The measurement
move itself — that a careful, standardized re-evaluation can *reorder* a field's leaderboard — has a
direct precedent outside metabolism: **Syntheseus** (Maziarz et al. 2023, *arXiv*:2310.19796; Faraday
Discuss. 2025, doi:10.1039/D4FD00093E) showed the ranking of state-of-the-art retrosynthesis models
changes when they are evaluated carefully. We extend that thesis with a **quantified, pre-declared
endpoint** — the match-protocol × method interaction with a paired bootstrap CI (§11, primary
endpoint) rather than an observation that the order moved — and ask, as future work (§12), whether
the reordering is itself a cross-domain phenomenon of molecular ML.
...
- ~~A paired CI for Proposition 1's Spike-3 listwise-reranker confirmation~~ — **RESOLVED by
  re-scoping, not by new compute.** The Spike-3 per-substrate scores are not in the tree
  (`results/reranker_gate_bi_test{,_seed1,_seed2}.json` are cited by `stage2_ranker_evidence.md` but
  absent), so a paired bootstrap for that number would require retraining. §10 now instead leads
  Proposition 1's confirmation with the **controlled objective swap on the full n=1170 test**, which
  already carries paired intervals: identical heads/features/pool, pointwise-trained add nothing
  (+0.0002 [−0.0073, +0.0077] n.s.) while listwise-trained gain +0.0089 [+0.0032, +0.0153] over them
  and +0.0091 [+0.0027, +0.0160] over the baseline (`results/joint_rerank.json`), corroborated by
  +0.0165 [+0.006, +0.027] in the hybrid arm (`results/hybrid_rerank_full1170.json`). Spike-3's
  +0.067 is retained as explicitly secondary evidence with its 3-seed spread and no fabricated
  interval. Still worth doing if the seeds are ever re-run: commit the three JSONs so the Stage-2
