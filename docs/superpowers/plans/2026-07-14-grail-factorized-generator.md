...
`build_factorized_dataset` iterates `molframe.map.items()`. For each `(sub, mets)`: build `data = from_rdmol(Chem.MolFromSmiles(sub))`; skip if None/empty. `num_types = 1 + max(rule_to_type.values())`. Init `y_type = zeros(num_types)`. For each rule in the bank, apply it to `sub` (RDKit `safe_run_reactants` via `grail_metabolism/utils/preparation.py`), and if any product tautomer-matches a metabolite in `mets` (reuse `grail_metabolism.metrics._tautomer_inchikey`), set `y_type[rule_to_type.get(rule, -1)] = 1` (ignore the `-1` bucket). Set `y_site = zeros(num_atoms)`, then for the union `derive_som_labels(sub, mets)` set those atom indices to 1. Attach both to `data`. Return the list. (This reuses the exact rule-application + tautomer-match already used by `MolFrame`, so it is consistent with the coverage ceiling.)
