...
DECLARED_DEBT = {
    "audit_artifact_provenance.py":
        "THREE pinned artifacts report producer_changed and none of the three can be made current "
        "without changing a number the paper prints. The gate was red with no entry here, which is "
        ...
        "results/benchmark_report_depth2.json. Newly visible, and visible because of a repair: its "
        "lift was a hand-typed literal on check_paper2_numbers.ALLOWED, the list for numerals that "
        "are part of the language rather than a result, while paper2/body.tex said no figure from "
        "the probe was quoted and paper2/si.tex quoted it. The figure now comes from the file "
        "through a macro and the manuscript states the weakness, which is what put the artifact in "
        "front of this sweep. It records no seed, so a re-run draws a different 150-substrate "
        "sample and returns a different lift: currency would change a printed number rather than "
        "confirm it.\n"
        ...
        "through infer(). CLOSES for the first two never, the obstacles being an upstream file "
        "nobody can regenerate and someone else's web service. The third is a producer that did not "
        "record its own invocation, which is a repairable thing and is being repaired.",
    "check_prose_rate.py":
        ...
        "LENGTH, the one red criterion. paper2/body.tex is 10,061 words against 11,131 at the start of the pass: 1,070 "
        "removed, 9.6 per cent, against a 25 per cent target of 8,349. The author accepted roughly "
        "-14 per cent and recorded the rest here; the pass reached -9.6, and the 488-word gap "
        ...
        "dropping an artifact. The only blocks large enough to cover 488 words are a worked "
        ...
        "the remaining 1,712 words were not taken, each figure measured: every sentence in the file "
        ...
        "environment bodies entirely, so NONE of its 209 ACS-mandated words are inside the 10,061 "
