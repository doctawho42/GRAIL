...
positives; rule-applicable products that carry no annotation are unlabelled rather than negative,
because a missing annotation does not establish that a transformation does not occur. The filter
is therefore trained on raw classifier outputs, so that a non-negative positive-unlabelled
surrogate is not applied on top of a sigmoid, and the generator down-weights applicable but
unobserved rules instead of penalising them as hard negatives.
...
The three stages are separately inspectable --- the selected rule, the enumerated product, and
the filter's judgement of the pair --- and interpretable rule selection paired with a
positive-unlabelled filter is what the architecture contributes, not recall above other
...
Optimisation uses Adam at a learning rate of $10^{-4}$ for at most 20 epochs with early stopping.
...
correspondence between atoms rather than by index order, so that chemically meaningful atom
...
Both variants use a non-negative positive-unlabelled risk estimator with a class prior of 0.75,
evaluated on logits. Optimisation uses Adam at a learning rate of $10^{-4}$ for at most 20 epochs
...
for the merged-graph variant, a paired difference of $-0.0090$ with a 95\% confidence interval of
$[-0.017,-0.001]$ over $n=\ntest$; at recall@5 the margin widens, 0.282 against 0.259.
...
computation at every candidate at inference. Accuracy and cost point the same way, so the
merged-graph variant is not merely unused but dominated.

\subsection{How the rule bank was built}

The expert systems GRAIL is compared against, such as SyGMa and Meteor, use hand-authored reaction
...
locates a ring-aware maximum common substructure as an anchor, using RDKit's FMCS with ring atoms
matched only to ring atoms, rings completed, any bond order allowed to match, and atoms compared
by element; the anchor must cover at least 40 per cent of the smaller molecule's atoms. From that
anchor it derives a reaction centre. Three kinds of atom are flagged: matched atoms whose element,
...
substrate--product match combinations are tried, and the one giving the smallest centre is kept. The centre is then
...
applicable to more than 90 per cent of sampled substrates while still averaging more than 50
products.
...
unique self-tested templates from the training positives. Of these, 4{,}274 --- 73 per cent ---
...
templates rather than by broad general reactions.
