...
Optimisation uses Adam at a learning rate of $10^{-4}$ with early stopping: the released
preset budgets $20$ epochs and the deployed generator trained for $8$.
...
filter under the second. Optimisation uses Adam at a learning rate of $10^{-4}$ under the same budget, the deployed
filter having trained for $8$ epochs
...
They also win on cost. Encoding a substrate and a product as one merged graph costs
$3.75\times$ what encoding the two independently costs --- $34.0$\,ms against
$9.1$\,ms per pair on the released candidates --- because it computes a maximum
common substructure for every pair, and it pays that at every candidate at inference as well as at
every training pair. Accuracy and cost point the same way. The merged-graph variant is implemented and available, and is not deployed because it loses on both counts.
...
of 474, 656, 500 and 1{,}051 rules, together with the newly mined templates.
