...
Under that common budget SyGMa leads GRAIL at every $k$ with no crossover: 0.233 against 0.127 at
$k=1$, 0.493 against 0.350 at $k=15$, and 0.520 against 0.410 at $k=64$. GRAIL does not win at a
matched output budget, and its selector and ranker are the binding weakness.
...
values below come from the same re-scoring run as the frontier.

SyGMa has no selection stage: it applies its whole applicable rule set and emits the result, at
about 64 outputs per substrate and precision 0.076. Almost nothing is lost between what its pool
...
Nothing is lost at the cut-off itself: emitting about eight candidates, GRAIL never reaches the
budget of fifteen, so its recall@15 equals its pool coverage exactly.
