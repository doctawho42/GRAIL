...
thresholds, which is the claim; $0.5$ is where it is largest and is reported as such rather than
as a tuned value.
...
(\S\ref{sec:robust}). On the $845$ parent drugs the rule reaches $0.236$ against the best constant's
$0.215$; on the $325$ substrates that are themselves metabolites of another substrate here it
reaches $0.140$ against $0.141$, so on the stratum where the model is weaker its scores say nothing
about how many to emit. Under strict \textsc{inchikey} matching rather than the tautomer-aware
default the rule reaches $0.205$ against $0.190$. The result is a property of one method's frozen
scores on one corpus: it says that this pool is informative about this set size, not that relative
emission is generally preferable.
