...
instead. The aggregate is a second undeclared parameter and the more consequential one, since the
method that leads on recall finishes last on F1, and the decomposition predicts that reordering
from a structural property rather than observing it. We release the protocol, the audited split and
...
the F1 ordering while leaving recall's stable to within noise. A third parameter is the aggregate
...
Against the published methods on the full clean test split, GRAIL trails both: SyGMa reaches
$0.572$, a paired difference of $-0.242$ $[-0.271,\,-0.212]$ over the $1{,}168$ common substrates,
and MetaPredictor $0.568$ on the same split ($0.585$ on the shared subset). Nor does GRAIL lead at
any output budget: a frontier sweeping $k \in [1,64]$ finds SyGMa ahead at \emph{every} $k$ with no
crossover (Appendix~\ref{app:budget}), so the gap is not output-size accounting but the selection
weakness \S\ref{sec:decomp} localises. The two differ on a second axis besides, returning $8.4$
candidates per substrate against $81.7$ --- an operating point recall@$k$ does not express, and
\S\ref{sec:metric} prices.
...
having a selection stage. As an aggregate those numbers say only that one scores higher.

\looseness=-1
Two free parameters sit between a set of predictions and a leaderboard position, and this field
declares neither. The first is the rule for when two structures are the same molecule, not neutral
because methods differ in how their outputs depart from the references. The second is the
aggregate: the method leading on recall finishes last on F1, which the decomposition predicts
structurally, a pipeline with no selection stage being what recall rewards and precision punishes.
The two halves of the paper meet there. A score is interpretable only alongside the criterion and
the aggregate that produced it, and both belong in any report, with the coverage ceiling wherever
there is a rule base.
