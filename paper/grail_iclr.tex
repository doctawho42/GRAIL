...
paired bootstrap when we analysed the results, and we apply no correction for testing six method
pairs. The reversals are therefore reported as illustrations of the mechanism rather than as
independently significant findings. It is significant on two independent
...
magnitude across them, and three of the four intervals exclude zero. Of the six method pairs, four show a significant interaction (reported without correction for multiple comparisons),
...
\paragraph{Reading GRAIL's numbers.} Three figures appear and are not interchangeable: $0.365$ is
the paired 150-substrate subset, $\grailmacro$ the mean of per-substrate recalls on the full split,
and $\realised$ the pooled ratio of hits to references, lower because substrates with many
references weigh more. The decomposition uses the pooled form, the only one that multiplies out
exactly.
...
per-substrate scores were not retained and which is flagged where it appears. The release contains
every method's frozen predictions, the leakage-audit report, the evaluation harness with its five
criteria, the split construction and audit, the analysis code, and the GRAIL checkpoints, in an
anonymized archive accompanying this submission.
