...
this protocol to published methods and to our own, we find that the matching criterion
...
have their \emph{frozen} predictions re-scored by the TAME harness under all five of its criteria, so
...
criterion, so only the differential can reorder anything. We did not register this endpoint before
seeing the data, and we do not correct for testing six pairs; the reversals below are illustrations
rather than independently significant findings. It is significant on two independent
...
per-substrate scores were not retained, flagged where it appears: the frozen predictions of
every
