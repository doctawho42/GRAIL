...
Standardisation is the binding constraint (Section~\ref{SI-sec:si-runtime}). Three other
explanations were tested and rejected by measurement: the filter's maximum-common-substructure
step, rule application, and substrate size as an axis, which Figure~\ref{fig:cost} plots against
generator seconds. The cause was product standardisation, at
...
same evaluated test set, leaving \numCeilingUncovered{} uncovered. That count is measured
through the firing loop a caller runs, with hydrogens implicit; a loop and a hydrogen convention
together are the \emph{instrument}, and the bound moves with it. A second loop, which audits both
conventions from one code path, counts \numCeilagreeAuditimplicitUncovered{} under that same
convention, and four counts of this bound exist in all. They are set out below rather than left
for a reader to discover, because the number the rest of this section is about is the one that
moves with the instrument. It carries no
