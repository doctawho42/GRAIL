...
behind several withdrawals. Knowing which metabolites will form, as chemical structures rather than as a count or a rate, is
...
has to be defended to a medicinal chemist. A sequence model is not built to
...
reorder the models. The multiverse and specification-curve discipline
\citep{Steegen_2016, Simonsohn_2020} reports the whole space of defensible analysis
choices; what follows applies it to metabolite prediction, where the
two choices that move the ordering are how a match is judged and how much a system may return. A protocol tells an author
...
candidates. We evaluate it with the two decisions above promoted to declared axes: we name each
and sweep it across its full range, where both are otherwise fixed in silence. The comparison is against
\numComparatorsColumnsWord{} published predictors we obtained and ran on the same substrates.\section{Materials and methods}
\label{sec:methods}
...
remainder needs. The \emph{validation draw} is a declared sample of \numValdrawCap{} validation records, holding \numValdrawDeclared{} distinct substrates of which \numValdrawPaired{} carry a whole-bank pool. The registered ranking and timing thresholds are checked on it, because a threshold checked on the population that chose the setting it tests checks nothing; Table~\ref{tab:hyp} records the population each prediction was in the event checked on, and Section~\ref{SI-sec:si-register} states that all but one of the ten fixed a threshold in advance while leaving the population open. The
...
That third population has a defect that has to be stated before any number is read on it. It is no
...
over SyGMa at a budget of fifteen does not, so that one is restricted to the comparison set everywhere
it appears. An \emph{arm} here is one column of the comparison, and
...
MetaTox and MetaPredictor publish no development set that could be intersected with it.
\subsection{Overview}
...
not whether a reference is reached. A scorer scores each
(substrate, candidate) pair, encoding each of the two molecules in its own graph, and the candidates are ordered by a rank fusion of that scorer's and the generator's
orderings.
...
Both learned stages were trained on a subsample of the training split, fixed when the run was configured, before anything reported here was measured, so what the ablations compare (Section~\ref{SI-sec:si-ablations}) is the value of each stage at that training budget, which is smaller than the split allows. Every setting is in Section~\ref{SI-sec:si-arch}.
...
of metabolite structures that is a restriction on what may be asked of it, not a
...
and truncation can therefore raise recall as well as bound it. The cap raises recall by $+\numHNineDiff$ $[+\numHNineLo, +\numHNineHi]$ and takes the mean pool from \numHNinePoolBefore{} candidates to \numHNinePoolAfter{} (P2, Table~\ref{tab:hyp}). The cap must be at
...
return; where this paper writes \emph{budget} unqualified it means that one. The \emph{interactive} mode applies the \numHOneZeroTopk{} templates the trained
...
the released checkpoint was trained, which is a fact about the run's configuration. The
number stands on the validation curve measured either side of it
...
On the comparison set the same quantity is $+\numHOneZeroComparison$, past the ceiling. The two
populations disagree because at $k \le 10$ the whole bank
buys nothing on either, and from $k=15$ it buys more the wider the budget, because the
interactive arm runs out of candidates. P3's ceiling is therefore a property of the output budget
requested, not of the system, and so is the deployed rule budget. Read at an output budget of fifteen the curve is flat past the deployed rule budget; read at an output budget of thirty it is not (Section~\ref{SI-sec:si-budget}).
...
arm alone. It was registered as prediction P4 and confirmed (Table~\ref{tab:hyp}); its budget ladder was fixed with the prediction and is not the one read elsewhere here.
...
most conjugation are predicted as constitutions, with configuration left unspecified. All five are reported for every
...
neutral. It is a deployment question as much as an evaluation one, because a user submits the drawing a chemist would make, and on the substrates the standardiser changes the effect is larger than the pooled figure suggests (Section~\ref{SI-sec:si-dialcond}).
...
The arms did not all meet that choice the same way, and no sweep here removes the asymmetry.
...
(Section~\ref{SI-sec:si-singlestep}). GLORYxR returns both phases and its site-of-metabolism
setting is a documented knob, so both of its settings are run and both are printed. The depth is
swept for SyGMa, whose scenario is a documented setting, and the lead survives it. BioTransformer's
step count is also ours to turn, and it is swept here. A second generation returns a candidate set
that contains the first's on every one of the \numBtstepsPaired{} substrates where both runs
finish, losing \numBtstepsLostgenoneWord{} of the structures one step found, and emits
\numBtstepsRatio{} times as many candidates; it adds nothing at the two tightest budgets and
\numBtstepsGainfifty{} at the widest. The knob does not reach past the output budget, and on
\numBtstepsLost{} of \numBtstepsAttempted{} substrates the second generation does not complete at
all (Section~\ref{SI-sec:si-comparators}). For those two the difference between the arms stands, and the wide-budget cells should be read with it in view.
...
$97.5$ percentiles of $\{\delta^{(b)}\}$. Table~\ref{SI-tab:si-precision} carries precision, which does not order the systems, because under incomplete annotation a real but unannotated metabolite counts as a false positive against whichever arm predicts it (Section~\ref{SI-sec:si-precision}).
...
This paper prints more contrasts than that family covers. Applying the same
...
\numHolmWideleadsremovedWord{} is a lead of this work's, over MetaTox at $k=10$, and the other four
run against it: the exhaustive arm's trails against SyGMa at $k=1$, $3$ and $5$, and the
interactive arm's against SyGMa at $k=1$. So the wider reading costs this paper one cell and
gains it four, and neither reading changes the wide-budget result.
...
(green); budgets where neither separates are unshaded. The names along the top give the strongest
comparator over each stretch of budgets, which is the arm the shading there is read against: four
systems hold that position across the sweep. The shading is read at a nominal budget,
...
in three. Every difference behind that division, with its paired interval, is
...
not survive the family-wise correction either.  \numSygmaInside{} of SyGMa's
...
it at $3$ and $5$; the pattern is not monotone in the budget. The
...
difference is
$\numBtarmMatchedgap$ $[\numBtarmMatchedlo, +\numBtarmMatchedhi]$, an interval covering zero.
\input{si_table_matched}
The corrections that follow concern the sweep, separately from
the matched-length table above (Table~\ref{tab:si-matched}): the \numSweepWideleadsWord{} cells in which the exhaustive arm leads a comparator
...
comparators, so the correction runs over the \numSweepWideleadsinfamilyWord{} it covers; of
...
as a direction only; the caution
...
a limit on ranking, not on coverage: the wider pool contains candidates the smaller budget never
produced, and on those classes they displace correct answers out of the window.

A nominal budget does not fix a length, and the leads at wide budgets are read where most comparator
...
Three corrections apply to the wide-budget leads.
...
and the one against SyGMa does not. And SyGMa met the corpus's drawing, which differs from the one the
...
The exhaustive arm's lead is at depth in the list. MetaPredictor is flat from $k=15$
...
because it keeps having candidates. At $k=50$, of
...
How the per-substrate figures are pooled is a third choice, and it is swept.
Every figure in Table~\ref{tab:sweep} is micro,
...
over (substrate, candidate) pairs competes with: similarity to the
substrate, which needs no training and no labels. The fusion beats it by $+\numSimCmpGapOneFive$
$[+\numSimCmpLoOneFive, +\numSimCmpHiOneFive]$ at every budget on both populations. The substrate-specific part of the rule choice is worth less than either. Against the same number of applicable rules picked by training frequency it gains a small margin that separates at every budget (Section~\ref{SI-sec:si-ablations}).

Two registered mechanisms failed. Emitting candidates in whole groups failed (P6), and so did gating on the same signal before the fusion (P7). The second is closed by an oracle: even
...
measurement: a confirmation would only restate the ordering it started from (Section~\ref{SI-sec:si-oracle}).

One trained checkpoint produced every figure here, so the intervals quantify the population
with the training held fixed. Retraining both stages under \numSpreadSeedsWord{} seeds at the configuration these tables report measures the training noise, and against the interactive arm's spread the rank fusion of P1 is \numSpreadPOneInsd{} times it, the pool cap of P2 \numSpreadPTwoInsd{} times it, and the rule budget of P3 \numSpreadPThreeInsd{} times it, which places P3 outside that spread. The last of these is indicative only (Section~\ref{SI-sec:si-retrain}).

\subsection{How much of the verdict is the criterion}

The whole comparison is recomputed under each of the \numCritCriteria{} declared criteria, holding
the population, the arms, the ranking and the parent-drop convention fixed. The verdict moves at up
to \numCritMovedMax{} of \numCritBudgets{} budgets (Figure~\ref{fig:criterion}). Neither the arm
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{fig_criterion}
\caption{The verdict of the comparison under each declared matching criterion. $+$ marks a budget
where GRAIL's better arm leads the strongest comparator with the paired interval excluding zero,
$-$ one where it trails on the same terms, and $\cdot$ one where the interval covers zero. The
superscript names the comparator the cell is read against, M for MetaTox, S for SyGMa, P for
MetaPredictor, G for GLORYx, and Rd or Rs for GLORYxR in its default or strict site-of-metabolism
setting. The row beneath names the GRAIL arm, \emph{int.} or \emph{exh.},
with a dash where the better arm is not the same one under all five criteria at that budget.}
\label{fig:criterion}
\end{figure*}
...
candidate can therefore be rejected on mechanistic grounds. Predicting the
...
its own labels are derived \citep{Jacob_2025_autosom}. The claim here is narrower and concerns
consistency: the site this system names is the site that changed.
...
audited this way. The audit is run at a rule budget of \numSiteRulebudget{}, and the whole bank
would give a different population of scored matches.

How much a chemist gets from an attribution also depends on the template: \numRsizeLethreesharePct{} of the parseable templates have three reactant atoms or fewer, and there the site discrimination comes from the learned stages (Section~\ref{SI-sec:si-bank}).
...
under both drawings of the substrate. The rank axis compresses above $k=30$, where the ticks are
closer together than they are wide and read as a block.}
\label{fig:case}
\end{figure*}
...
\numCaseExhFoundWord{} are recovered only beyond $k=15$, so at the reported budget this molecule counts as one hit of the four. The
...
with; the tenth was measured, not adjudicated, because its threshold was fixed in advance
...
afterwards. Section~\ref{sec:limitations} states which of the six confirmations a stricter
...
The binding constraint is product standardisation, run once per product of every template, and the cost it sets is in Section~\ref{SI-sec:si-runtime}. Two registered predictions followed. Standardising only the survivors (P8) freed recall but missed its registered speed-up threshold (Table~\ref{tab:hyp}). Adding a
...
On the comparison set the ceiling is \numHeadroomCeiling{} and the exhaustive
...
The ceiling counts one application of one template, where an enzymatic step may take several, and that matters for how the number reads. The
annotation itself is not all single steps: applying the composite instruments to the references themselves, \numRefstepEither{} of the \numRefstepTyped{} pairs those
...
fourth arm, hydrogens made explicit without completing the templates, is an incomplete loop and leaves \numHydUncoveredexpanded{} uncovered, more than any of the other
...
which \numCensusOnce{} occur once and carry \numCensusOnceSharePct{} of the mass. A reader may put
this down to how finely a type is defined; varying the definition answers that. What
that answer covers should be said exactly: it covers the \numCeilingNovel{} misses of absent type, and none of the other \numCeilingKnown{} plus \numCeilingUntypeable{} of the
...
meaningful. That makes the census of the gap a fact about the annotation corpus, independent of our own choice of definition; what the engine contributes is the hydrogen convention measured
above.
...
SyGMa's lead at the tight budgets is a result about the field, which this system only inherits. SyGMa is the
...
A single recall figure at a single budget would have named a different winner depending on which
budget its author chose, and each of those choices would have been defensible.

\subsection{Choosing an operating point}
\label{sec:offers}

The two modes are two settings of one rule budget, and the budget the service answers at decides
which to run. Below about ten candidates the interactive mode is the right setting, and the
faster of the two by the margin Figure~\ref{SI-fig:cost} reports.

Beyond a short list that setting is the weaker one, and the cause is the pool, whose size bounds what any ranking can return. It returns far fewer candidates than MetaTox, and by $k=20$ many of its lists have already ended; a list that has ended cannot gain a hit from a wider window, so it trails MetaTox at every budget above fifteen with the interval excluding zero (Tables~\ref{SI-tab:si-short} and~\ref{SI-tab:si-intervals}). A service deployed in that mode would recover
...
turns to its limit. On validation that gain requires an output budget of thirty (Section~\ref{SI-sec:si-budget}). Either way it pays
...
reporting one is the norm here, not a contribution of this work. Checking it against the atoms that actually
...
wide budgets is partly a statement about output length on all sides, independent of ranking.

This paper turned its argument about other people's figures on its own release, and it
found settings that decide a number and were not declared. Six of them: the aggregation of the templates reaching one candidate, the fusion constant, a rule gate the released checkpoint carries, a set of pools scored by a checkpoint this repository no longer ships, the drawing of the substrate, and the pool cap. Each is declared and measured here.

\subsection{The ceiling bounds this formulation, not the task}

The shortfall is a property of the corpus the templates are mined from, and it is measured directly: of the \numCeilingUncovered{} references one
application of this bank fails to reach, \numContainmentAbsent{} are of a type the training
annotation never holds, which is \numContainmentShortfallsharePct{}. The join behind that figure is
measured, and it falls inside the interval its own marginals allow, \numBoundShortfallsharemin{}
to \numBoundShortfallsharemax{}; Section~\ref{SI-sec:si-typeoverlap} gives the counts both rest on.
...
What the bound licenses has to be said plainly, because the corpus it is a property of has two
...
The narrower statement still earns its place, because this corpus is assembled from the
...
therefore a one-template ceiling, where a single enzymatic step may take several templates, and the composite share is why a second
application of the bank buys little: a fifth of the mined bank already carries the compositions a
second pass would have to discover. A depth-two probe put the lift at \numDepthtwoLift{} on
\numDepthtwoSubstrates{} substrates, at \numDepthtwoCost{} times the candidate cost. Its result
file records the producer that wrote it, the seed and sample that drew those substrates and the
digests of its inputs, and re-running that producer reproduces every figure it carries.

Both learned stages are trained short of what the split allows, on a subsample of it and with early stopping that never engaged, which is an underfitting signature; retraining on the full split is not reported (Section~\ref{SI-sec:si-retrain}).
...
threshold. The stricter reading asks the whole interval to clear it. P1 and P2 have
...
afterwards. Table~\ref{tab:hyp} records that as a measurement, and so
...
ran; the choice is a parameter, so the blend can be selected without
...
(Section~\ref{SI-sec:si-comparators}). Stereochemistry and protonation are bounded; what they cost is left open. \numStereoSharePct{} of annotated
...
for one to read. What separates the two settings that differ in that layer, on this annotation, is protonation alone (Section~\ref{SI-sec:si-sensitivity}).
...
establishes an ordering within a corpus, which is weaker than a ranking of the methods.
...
benchmarked returned, so what each emitted is counted directly. Counted, across their
...
with the rank fusion gained more recall than any architectural change tested here.
...
Enlarging this bank from this corpus would not supply that chemistry. The bound is therefore a property of the corpus this field trains on, and every bank inherits it.
...
buys and two parameters of the ranking, swept; four trained variants of the ranking, three on one
frontier and one below it; architecture, training and settings of both learned
...
least one, which a check reports and the repository does not merely assert. The repository's
licence is \RepoLicence{}, which is the only licence the remaining GPL templates permit, and \texttt{NOTICE.md} carries the decisions file by file.
...
be obtained cannot be compared against, and for that reason one of the \numComparatorsUnavailable{}
systems of 2025--26 is excluded here. GLORYxR is in the comparison and clears that bar for us but
not for a reader: its weights are undistributed and were supplied privately for this work, so nobody can
recompute its two columns from public sources. Section~\ref{SI-sec:si-comparators} says so
beside them and records the digest of every weight file, so a deposit its author intends to make can
be checked against what ran.
...
financial or otherwise. Any interest that relationship could create runs against this work, not
for it, and the Supporting Information sets out what the MetaTox arm consists of, together with
what the service does not expose to a submitter.
