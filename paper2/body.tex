...
mechanistic grounds rather than on a score. How much mechanism that carries depends on the template. \numRsizeLethreesharePct{} of the bank,
\numRsizeLethree{} of \numRsizeParsed{} parseable templates, is patterns of three atoms or fewer,
for which the named site reports where the pattern matched rather than a mechanism.
Section~\ref{sec:offers} measures both the size of that part and how often the named site is the
site that changed. We evaluate it with the two decisions above promoted to declared axes: each is stated and varied
across its full range instead of being fixed silently. The comparison is against
\numComparatorsColumnsWord{} published predictors we obtained and ran on the same substrates, with
the two whose knowledge is built on a source of this corpus named and their overlap measured. One system is absent and its absence was not
...
the four sources is not recoverable. The corpus files predate this repository's first commit, no script in it reads the four sources,
and the accession field is empty in every record. The apparatus built on top therefore makes every
...
and truncation can therefore raise recall rather than only bound it. It was registered as
prediction P2 with a threshold of $+\numHNineThreshold$ and measured $+\numHNineDiff$
$[+\numHNineLo, +\numHNineHi]$, taking the mean pool from \numHNinePoolBefore{} to
\numHNinePoolAfter{}. The value was fixed by a rule rather than by a search. The cap must be at
least twice the widest budget reported, so that the widest column is never simply the whole pool,
...
(Section~\ref{SI-sec:si-budget}). What the difference buys was registered as prediction P3, with a ceiling of
$+\numHOneZeroThreshold$. On validation it is $+\numHOneZeroBought$, 95\% CI
$[\numHOneZeroLo, +\numHOneZeroHi]$, taking the mean pool on that draw from
...
\numSweepSygmaOne{} against \numSweepTrainedOne{} at $k=1$. Two of those three, at $3$ and $5$, do
not survive the family-wise correction either. That is marked in
Table~\ref{SI-tab:si-intervals} and stated here rather than later, because the correction is
applied to the leads below and should be applied to the trails in the same breath. The two banks
...
\numThirdpartyBiotransformerShipped{} of this bank's templates. Cut to its own list length, the exhaustive arm does not lead it at all: the difference is
...
therefore falls in a class none of the six systems is built for. That is what a reader deciding
whether any of them is usable on their own compounds needs before they read a headline number.
...
once. Micro is the ratio of sums; macro is the mean of per-substrate recall, which weights a substrate
carrying one reference like one carrying twelve. Every figure in Table~\ref{tab:sweep} is micro,
...
disagree on, one of them a lead claimed here. The choice does not rescue the budget
...
$[+\numSimCmpLoOneFive, +\numSimCmpHiOneFive]$ at every budget on both populations. The substrate-specific part of the rule choice is worth less than either. Against the same number
of applicable rules picked by training frequency it gains $+\numSelVspriorApplicable$
$[+\numSelVspriorApplicablelo, +\numSelVspriorApplicablehi]$: small at every budget, and separating
at all of them. It is reported as that rather than as the stage's justification
...
unstated. It is an argument about published numbers, and it applies to ours. The whole comparison is recomputed under each of the \numCritCriteria{} declared criteria, holding
...
InChIKey. That is a range of \numCritSwingOneFive{}, and the sign of the conclusion changes with
it. That
range is the claim. The cell-by-cell verdicts in the table are read from per-comparison intervals with no correction
...
the point estimates and depends on no multiplicity convention. Under
...
It is measured on the interactive arm, whose rule budget of \numSiteRulebudget{} the audit records.
...
\numSiteInsidePct{} (Section~\ref{SI-sec:si-site}). The null is a connected fragment of the same
...
\numSiteCuratedscored{} matches carried by a hand-written template, for which they are independent,
agree at \numSiteCuratedPct{} against \numSiteMinedPct{} for the mined ones. The claim therefore
does not rest on the templates whose provenance would guarantee it. The exhaustive arm is not
audited this way, because it applies the whole bank rather than a budget of \numSiteRulebudget{}
rules, and the population of scored matches would be a different one.

How much a chemist gets from an attribution also depends on the template.
\numRsizeLethreesharePct{} of the parseable templates have three reactant atoms or fewer, and
\numRsizeCasetop{} of the twenty highest-ranked candidates in the worked example come from
templates that small, where the site discrimination comes from the learned stages rather than from
the rule.
...
reading a short list is working at exactly those budgets. That a rule set of this size is not beaten at the head of the list by any of the systems here, two
of them released since, is worth stating plainly. It is visible only because the budget is swept.
At a budget of fifty the same table has this work's exhaustive arm ahead of SyGMa by
$+\numGapBanksygmaFiveZero$, and ahead of every comparator by at least
$+\numGapBankmetatoxFiveZero$: the ordering at the head of the list reverses.
...
A service answering with thirty or fifty must therefore either run the exhaustive mode, which leads
MetaTox at both, or raise the rule budget, which is the same knob. On validation, raising it to the
whole bank is worth $+\numBudgetKSevenFiveEightOneGapthirty$
...
The register is weaker than its count of confirmations suggests. Three of the seven confirmed predictions clear the falsification condition as written, which
compares a point estimate to a threshold, and not the stricter reading that asks the whole interval
to clear it. P1, P2 and P5 each have their threshold inside their own interval. What the data
establish for those three is that the effect exceeds zero, not that it exceeds the registered
bar. P5 is weaker again, and
...
price a configuration-aware criterion, because two of its five settings return the identical
verdict at every budget: the annotation has no configuration for them to disagree about
(Section~\ref{SI-sec:si-sensitivity}).
