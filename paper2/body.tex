...
reorder the models. The gap between the two is where this work sits. Reporting the whole space of defensible
analysis choices instead of one chosen cell is the multiverse and specification-curve discipline
\citep{Steegen_2016, Simonsohn_2020}; what follows applies it to metabolite prediction, where the
...
continuous setting, and the two modes reported throughout are two points on it. The \emph{interactive} mode applies the \numHOneZeroTopk{} templates the trained
...
The exhaustive arm leads at depth rather than at the head of the list. MetaPredictor is flat from $k=15$
...
One trained checkpoint produced every figure here, so the intervals quantify the population and
