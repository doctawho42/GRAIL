...
the implementation offers (Section~\ref{SI-sec:si-knobs}). The blend of Equation~\ref{SI-eq:blend} is
...
two of the budgets a lead is claimed at, and that reading is in Section~\ref{sec:limitations}. Equation~\ref{eq:noisyor}
...
\numHNinePoolAfter{}. The value was fixed by a stated principle rather than by a search. The cap must be at
...
of fifteen the curve is flat past a rule budget of \numBudgetDeployed{}, which is where the
trained value happens to sit; read at an output budget of thirty it is not, and the whole bank gains $+\numBudgetKSevenFiveEightOneGapthirty$
...
(Section~\ref{SI-sec:si-aggregation}). Every other number in this paper is the released
aggregation's.
...
the lowest arm at the tight budgets, where MetaTox is, and it is the arm a reader of
...
Reading MetaPredictor's flatness off its list length infers the cause; its beam is the knob that
tests that, and turning it up is the question this work asks of other papers. At a wider beam its mean
...
MetaTox and the interactive arm do not emit alike either: MetaTox returns
\numEmittedMetatox{} candidates on average against the interactive arm's \numEmittedTrained{}, and at $k=15$
...
found settings that decide a number and were not declared. The aggregation that combines the
