...
(substrate, candidate) pair, encoding the two molecules as separate graphs and not as one
...
Both learned stages were trained on a subsample of the training split, fixed when the run was configured and not a consequence of anything measured here, so what the ablations compare (Section~\ref{SI-sec:si-ablations}) is the value of each stage at that training budget and not at the split's full size. Every setting is in Section~\ref{SI-sec:si-arch}.
...
fourth arm, hydrogens made explicit without completing the templates, is an incomplete loop and
not a convention and leaves \numHydUncoveredexpanded{} uncovered, more than any of the other
...
meaningful. That makes the census of the gap a fact about the annotation corpus and not about
...
SyGMa's lead at the tight budgets is a result about the field and not about this system. SyGMa is the
...
Beyond a short list that setting is the weaker one, and the cause is the pool and not the
...
wide budgets is partly a statement about output length on all sides and not about ranking.
...
ran; the choice is a parameter and not a retraining, so the blend can be selected without
...
(Section~\ref{SI-sec:si-comparators}). Stereochemistry and protonation are bounded and not priced. \numStereoSharePct{} of annotated
...
benchmarked returned, so what each emitted is counted and not inferred. Counted, across their
...
licence is \RepoLicence{}, which is what the remaining GPL templates permit and not a
